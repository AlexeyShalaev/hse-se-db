    ...
    async def seed_olympics(self, n):
        ...
        for _ in range(n):
            try:
                ...
                startdate = (
                    self._faker.date(before_today=False, after_today=True)
                    if year == now_year
                    else self._faker.date_this_century()
                )
        ...
        for _ in range(n):
            try:
                event = Event(
                    ...
                    num_players_in_team=(
                        self._faker.random_int(min=2, max=5) if self._faker.random_int(min=0, max=1) == 1 else None
                    ),
                    result_noted_in=self._faker.word(ext_word_list=["seconds", "meters", "points"]),
                )
                await self._db.insert(event)
            except:
                pass
        ...
        for _ in range(n):
            try:
                ...
                valid_players: list[Player] = [
                    p for p in players if p.birthdate.year <= olympic.year
                ]  # Players who were born before the event's year
